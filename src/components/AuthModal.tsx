        ...
        {user ? (
          ...
          <div className="space-y-5">
            <div className="flex items-center gap-3 pb-4 border-b border-gray-100">
              {user.photoURL ? (
                <img src={user.photoURL} alt={user.displayName || 'User'} className="w-12 h-12 rounded-full object-cover border-2 border-[#fe8028]" />
              ) : (
                <div className="w-12 h-12 rounded-full bg-[#001e40] text-white flex items-center justify-center font-bold text-lg">
                  {user.displayName ? user.displayName.charAt(0).toUpperCase() : 'U'}
                </div>
              )}
              <div>
                <h3 className="font-bold text-base text-gray-900">{user.displayName || 'Pengguna'}</h3>
                <p className="text-xs text-gray-500">{user.email || 'Akses Google Terverifikasi'}</p>
            ...
            <div>
              <div className="flex items-center justify-between mb-3">
                <h4 className="font-bold text-sm text-[#001e40] flex items-center gap-1.5">
                  <Bookmark className="w-4 h-4 text-[#fe8028]" />
                ...
                {bookmarkedArticles.length === 0 ? (
                  ...
                  bookmarkedArticles.map((art) => (
                    <div
                      ...
                      className="flex gap-2.5 p-2 rounded-lg hover:bg-amber-50 cursor-pointer border border-gray-100 transition-colors"
                    >
                      <img src={art.imageUrl} alt={art.title} className="w-12 h-12 rounded object-cover flex-shrink-0" />
                      <div className="flex-grow min-w-0">
                        <h5 className="font-bold text-xs text-gray-800 truncate">{art.title}</h5>
            ...
            <div className="pt-3 border-t border-gray-100 flex justify-between items-center">
              ...
              <button
                onClick={onClose}
                className="bg-[#001e40] text-white px-4 py-1.5 rounded-lg text-xs font-bold"
              >
                Tutup
              </button>
            </div>
          ...
          <div className="text-center space-y-5">
            <div className="w-12 h-12 bg-[#001e40]/10 text-[#001e40] rounded-full flex items-center justify-center mx-auto">
              <LogIn className="w-6 h-6" />
            </div>

            <div>
              <h3 className="font-extrabold text-xl text-[#001e40] font-display">
                Masuk ke HALOJATIMNEWS
              </h3>
            ...
            <div className="space-y-3 pt-2">
              <button
                ...
                disabled={isLoading}
                className="w-full bg-white hover:bg-gray-50 text-gray-800 border border-gray-300 font-bold py-2.5 px-4 rounded-xl shadow-xs transition-all flex items-center justify-center gap-3 text-sm cursor-pointer disabled:opacity-50"
              >
              ...
              <button
                ...
                disabled={isLoading}
                className="w-full bg-[#001e40] hover:bg-[#003366] text-white font-bold py-2.5 px-4 rounded-xl shadow-xs transition-all text-xs cursor-pointer"
              >
