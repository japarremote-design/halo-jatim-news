    ...
    <div className="max-w-3xl mx-auto space-y-6 pb-16">
      ...
      {loading ? (
        <div className="flex justify-center py-16">
          <div className="w-8 h-8 border-4 border-[#001e40] border-t-[#fe8028] rounded-full animate-spin" />
        </div>
      ) : editing ? (
        <div className="bg-white border border-gray-200 rounded-2xl p-6 space-y-4 shadow-sm">
          ...
          <div>
            <label className="block text-xs font-bold text-gray-700 mb-1">Isi Konten</label>
            <textarea
              value={contentInput}
              onChange={(e) => setContentInput(e.target.value)}
              rows={12}
              placeholder="Tulis isi halaman di sini. Pisahkan paragraf dengan baris kosong."
              className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:border-[#001e40] outline-none resize-y"
            />
          </div>
          <div className="flex justify-end gap-3 pt-2 border-t border-gray-100">
            <button
              ...
              className="px-4 py-2 border border-gray-300 text-gray-700 rounded-lg text-xs font-bold hover:bg-gray-50 flex items-center gap-1.5"
            >
        ...
        <div className="bg-white border border-gray-200 rounded-2xl p-6 md:p-8 shadow-sm">
          <h1 className="text-2xl md:text-3xl font-extrabold text-[#001e40] font-display mb-6">
            {displayTitle}
          </h1>
